...
congestion1%>%
  summarise(mean(#해당 시간 입력)) #시간대별 하루 평균혼잡도
congestion1 %>%
  group_by(line) %>%
  summarise(m=mean(#해당 시간 입력)) %>%
  arrange(desc(m)) #시간대 호선별 하루 평균혼잡도
...
congestion1%>%
  summarise(mean(#시간대 입력)) #시간대별 하루 평균혼잡도
congestion1 %>%
  group_by(line) %>%
  summarise(m=mean(#시간대 입력)) %>%
  arrange(desc(m)) #시간대 호선별 하루 평균혼잡도
